...
const defaultPoolScopePrincipal = "pool.scope.bridge"
...
func ResourceScopeFromAccess(access resource.AccessScope) ResourceScope {
	return ResourceScope{access: ensureAccessPrincipal(access)}
}
...
func NewResourceScope(scopeID, tenantID string) ResourceScope {
	...
	access := resource.NewAccessScope(scopeID, defaultPoolScopePrincipal, opts...)
...
func (s ResourceScope) AccessScope() resource.AccessScope {
	return ensureAccessPrincipal(s.access)
}
...
// WithTenantOverride preserves the caller metadata while enforcing a tenant identifier.
...
func ensureAccessPrincipal(scope resource.AccessScope) resource.AccessScope {
	if strings.TrimSpace(scope.Principal) == "" {
		scope.Principal = defaultPoolScopePrincipal
	}
	return scope
}
